...
{
    public class MC4105Extracter : Organization4105Extracter
    {
        public MC4105Extracter(ComplaintUnitOfWork unitOfWork, Models.FeeCollection feeCollection) : base(unitOfWork, feeCollection)
        {
        }
            ...
            ComplaintRecord complaintRecord)
        {
            if (FeeCollection == null)
                throw new Exception("FeeCollection.SetMC4105 Error. FeeCollection is null");
            if (!complaintValue.BookingAmount.HasValue)
                throw new Exception(string.Format("FeeCollection.SetMC4105 Error. CaseId = {0} ComplaintValues.BookingAmount is null. ValueId = {1}",
            ...
            if (string.IsNullOrWhiteSpace(participantId))
                throw new Exception(string.Format("FeeCollection.SetMC4105 Error. CaseId = {0} PARTICIPANT_ID is null. BIN = {1}",
            ...
            var feeCollectionControlNr = Utils.GetFeeCollectionControlNr(Organization.MC,
            ...
            var adjustmentComponent = new AdjustmentComponent
            {
                ...
                FulfillmentDocumentCode = complaintStage.DocumentationIndicator.HasValue ? complaintStage.DocumentationIndicator.Value ? "1" : "0" : "0",
                MasterCardFunctionCode = complaintRecord.FunctionCode,
                DestinationInstitutionId = complaintRecord.DestinationInstitutionId,
                DateAction = complaintStage.StageDate.HasValue ? complaintStage.StageDate.Value.AddDays(-1).ToString("yyMMdd") : null,
            ...
            GoodFaithLetterRecord gflRecord)
        {
            ...
            if (!gflValue.FeeAmount.HasValue)
                throw new Exception(string.Format("FeeCollection.SetMC4105 Error. CaseId = {0} GoodFaithLetterValue.FeeAmount is null. ValueId = {1}",
                    FeeCollection.CaseId, FeeCollection.ValueId));
            if (!gflRecord.FeeTypeId.HasValue)
                throw new Exception(string.Format(
                    "FeeCollection.SetMC4105 Error. FeeCollectionId = {0} CaseId = {1} Cannot find FeeTypeId value in GoodFaithLetterRecord with RecordId = {2}",
            ...
            var feeCollectionControlNr = Utils.GetFeeCollectionControlNr(Organization.MC, FeeCollection.FeeCollectionId);

            MockData mockData;
            var mc4105 = LoadMasterCardFeeConfiguration(gfl.CountryCode, gflRecord.FeeTypeId.Value, out mockData);
            ...
            mockData.AdjustmentComponent.DateAction = gflStage.StageDate.HasValue
                ? gflStage.StageDate.Value.AddDays(-1).ToString("yyMMdd")
                : null;
            mockData.CaseID = gfl.CaseId;

            int exponent;
            mc4105._2 = pan;
            mc4105._7 = FeeCollection.InsertDate.ToString();
            mc4105._12 = FeeCollection.InsertDate.ToString();
            mc4105._4 = helper.AmountToString(currencyCode, gflValue.FeeAmount.Value, 12, out exponent);
            mc4105._49 = currencyCode.Numeric.ToString();
            mc4105._127_022 = mockData;
            ...
            RecoveryCardRecord recoveryCardRecord)
        {
            ...
            if (!recoveryCardValue.FeeAmount.HasValue)
                throw new Exception(string.Format("FeeCollection.SetMC4105 Error. CaseId = {0} RecoveryCardValue.FeeAmount is null. ValueId = {1}",
                    FeeCollection.CaseId, FeeCollection.ValueId));

            if (!recoveryCardRecord.FeeTypeId.HasValue)
                throw new Exception(string.Format(
                    "FeeCollection.SetMC4105 Error. FeeCollectionId = {0} CaseId = {1} Cannot find FeeTypeId value in RecoveryCardRecord with RecordId = {2}",
            ...
            MockData mockData;
            var mc4105 = LoadMasterCardFeeConfiguration(recoveryCard.CountryCode, recoveryCardRecord.FeeTypeId.Value, out mockData);

            mockData.AdjustmentComponent.FeeCollectionControlNr = feeCollectionControlNr;
            mockData.AdjustmentComponent.MessageText = memberMessageText;
            mockData.AdjustmentComponent.DestinationInstitutionId = recoveryCardRecord.DestinationInstitutionId;
            mockData.AdjustmentComponent.DateAction = recoveryCardStage.StageDate.HasValue
                ? recoveryCardStage.StageDate.Value.AddDays(-1).ToString("yyMMdd")
                : null;
            ...
            mc4105._7 = FeeCollection.InsertDate.ToString();
            mc4105._12 = FeeCollection.InsertDate.ToString();
            mc4105._4 = helper.AmountToString(currencyCode, feeAmountSum, 12, out exponent);
            mc4105._49 = recoveryCardValue.FeeCurrencyCode;
            mc4105._127_022 = mockData;
